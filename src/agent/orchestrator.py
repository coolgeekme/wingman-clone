...
class AgentOrchestrator:
    ...
    async def _default_llm_call(
        ...
        try:
            ...
            llm = self._llm
            if tools:
                # Use standard tool format for langchain
    ...
    def _heuristic_response(self, messages: list[dict], tools: list[dict]) -> dict:
        if messages and messages[-1].get("role") == "tool":
            ...
            return {"role": "assistant", "content": f"Here are the results: {tool_content}"}
        ...
        for tool in tools:
            ...
            if tool_name == "get_weather" and any(w in user_msg for w in ["weather", "temperature", "forecast"]):
                city = "Unknown"
                if "in " in user_msg:
                    city = user_msg.split("in ")[-1].strip().rstrip("?.")
                return {
                    "role": "assistant",
                    "content": "",
                    "tool_calls": [
    ...
    async def process(self, user_prompt: str, history: Optional[List[dict]] = None) -> AgentResponse:
        """Main agent loop: prompt -> (optional tool calls) -> response."""
        # Use provided history or fall back to local memory
        if history:
            # Sync local memory if frontend provides history
            pass
        else:
